...
The following code-chunk use a function to show us, whether there are and
where the NAs are.
...
works in the same time zone. For that we use "lubridate" which gives all
the data a proper time- and date stamp. The old loggers works with CEST
...
Because solar irradiance is probably the biggest problem, the error through
the night is deeper than through the day. Therefore we analyze the whole
day (24h), through the day (day-time 06:00 - 22:00) and through the night
...
We plot a Boxplot for our 24h data set

```{r}
# Boxplot new logger - reference station at Zollikofen (2m)
box_2m_24h <- boxplot_creater(zoll_combined, Anomaly_new_log_ref2, colnbr = 8,
...
box_2m_day <- boxplot_creater(zoll_combined, Anomaly_new_log_ref2, colnbr = 8, whole = FALSE,
                              ylab = c('Temperature Anomaly [K]'), xlab = c('Hour of the Day [h]'))
...
box_3m_day <- boxplot_creater(zoll_combined, Anomaly_new_log_ref3, colnbr = 10, whole = FALSE,
                              ylab = c('Temperature Anomaly [K]'), xlab = c('Hour of the Day [h]'))
...
density_2m_day <- densityplot_creater(zoll_combined, Anomaly_new_log_ref2, colnbr = 8, whole = FALSE,
                    ylab = c('Density'), xlab = c('Temperature Anomaly [K]'))
...
density_3m_day <- densityplot_creater(zoll_combined, Anomaly_new_log_ref3, colnbr = 10, whole = FALSE,
                    ylab = c('Density'), xlab = c('Temperature Anomaly [K]'))
...
It is very complicated to calculate the errors. Unfortunately, I was not
able to use any kind of a loop because the data are to diverse. This is why
you see this mess...
