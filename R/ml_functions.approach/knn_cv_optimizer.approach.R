...
parameter.extracter.knn.cv.a <- function(own.sequence, df.1, df.2, number.of.validation){
  ...
  for (i in own.sequence) {
    # Train the model
    mod.knn <- knn.cv.model.a(df.1, i, number.of.validation)
    current.k <- i
    current.rsq.1 <- extract.rsq.a(mod.knn, df.1)
    current.mae.1 <- extract.mae.a(mod.knn, df.1)
    add.rsq.1 <- c(add.rsq.1, current.rsq.1)
    add.mae.1 <- c(add.mae.1, current.mae.1)
    current.rsq.2 <- extract.rsq.a(mod.knn, df.2)
    current.mae.2 <- extract.mae.a(mod.knn, df.2)
